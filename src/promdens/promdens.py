...
class InitialConditions:
    ...
    def calc_field(self, pulse: LaserPulse) -> None:
        ...
        t_ft = np.arange(
            self.tmin - 20 * self.field_fwhm, self.tmax + 20 * self.field_fwhm, dt
        ...
        if integral > 0.01:  # empirical threshold which considers the spectrum has maximum equal to 1
            print(
                "  - WARNING: Pulse is too short and integral of E(t) is not equal to 0 - Maxwell's equations are "
    ...
    def pulse_wigner(self, tprime, de):
        ...
        integral = np.trapz(
            x=s, y=cos * self.pulse.calc_field_envelope(tprime + s / 2) * self.pulse.calc_field_envelope(tprime - s / 2)
        )
    ...
    def sample_initial_conditions(self, nsamples_ic: int, neg_handling: str, preselect: bool, seed=None, output_fname="pda.dat"):
        ...
        rnd_max = (
            np.max(self.tdm**2) * self.pulse_wigner(self.field_t0, de=self.field_omega + self.field_lchirp * self.field_t0) * 1.01
        )
        ...
        if preselect:
            preselected = (
                np.interp(x=self.de, xp=self.field_ft_omega, fp=self.field_ft) < 1e-6
            )  # considering field_ft is normalized and positive
        ...
        if self.nstates == 1:
            print(
                f"  - Selected {unique_states[0]} unique ICs from {self.nsamples} provided. Unique ICs to be "
                f"propagated:\n   ",
                *np.array(unique[0], dtype=str),
            )
        else:
            print(
                f"  - Selected {np.sum(unique_states)} unique ICs over {self.nstates} state from {self.nsamples} "
            ...
            for state in range(self.nstates):
                if unique_states[state] == 0:
                    continue
                print(
                    f"  - State {state + 1} - {unique_states[state]} unique ICs to be propagated: \n   ",
...
def plot_spectrum(ics: InitialConditions) -> None:
    ...
    for state in range(ics.nstates):
        axs[0].plot(
            ics.traj_index, ics.de[state] / ics.evtoau, color=colors[state], alpha=0.6, label=r"S$_\mathregular{%d}$" % state
        )
...
def plot_field(ics: InitialConditions) -> None:
    ...
    axs[0].plot(ics.field_t / ics.fstoau, -ics.field_envelope, color=colors[0], alpha=0.4)
    axs[0].fill_between(
        ics.field_t / ics.fstoau, ics.field_envelope, -ics.field_envelope, color=colors[0], label="Envelope", alpha=0.2
    )
    ...
    for state in range(ics.nstates):
        axs[1].plot(
            ics.spectrum[0] / ics.evtoau,
            ics.spectrum[state + 1] / np.max(ics.spectrum[-1]),
            color=colors[-1],
            linestyle="--",
            linewidth=1,
            alpha=0.5,
    ...
    axs[1].plot(
        ics.spectrum[0] / ics.evtoau, ics.spectrum[-1] / np.max(ics.spectrum[-1]), color=colors[1], label="Absorption spectrum"
    )
    axs[1].fill_between(
        ics.spectrum[0] / ics.evtoau,
        ics.spectrum[-1] * 0,
        ics.spectrum[-1] / np.max(ics.spectrum[-1]),
        color=colors[1],
        alpha=0.2,
    )
...
def parse_cmd_args():
    ...
    parser.add_argument(
        "-np", "--npsamples", default=1000, type=positive_int, help="Number of initial conditions generated with PDA."
    )
    ...
    inp_file.add_argument(
        "-eu", "--energy_unit", choices=ENERGY_UNITS, default="a.u.", help="Units in which excitation energies are provided."
    )
    inp_file.add_argument(
        "-tu",
        "--tdm_unit",
    ...
    field.add_argument(
        "-lch", "--linear_chirp", default=0.0, type=float, help="Linear chirp [w(t) = w+lch*t] of the field frequency in a.u."
    )
    ...
    parser.add_argument(
        "-p", "--plot", action="store_true", help="Plot the input data and calculated results and save them as png images."
    )
...
def main():
    ...
    if config.method == "pda":
        ics.sample_initial_conditions(
            nsamples_ic=config.npsamples, neg_handling=config.neg_handling, preselect=config.preselect, seed=config.random_seed
        )
        if config.plot:
            plot_pda(ics)
