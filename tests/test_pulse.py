...
def test_field_cos_old(make_pulse):
    lchirp = 0.01
    omega = 0.1
    pulse = make_pulse(lchirp=lchirp, omega=omega)
    t = np.array([0.0, 1.0, 10.0])

    cos = pulse.field_cos(t)
    cos_old = InitialConditions.field_cos(omega, lchirp, t)

    assert len(cos_old) == len(cos)
    assert cos[0] == cos_old[0]
    assert cos[1] == cos_old[1]
    assert cos[2] == cos_old[2]
