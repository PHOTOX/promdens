...
def test_pdaw_nai(tmp_path):
    ...
    for i in range(len(weights)):
        ...
        assert weights[i] == pytest.approx(
            reference[i], abs=1e-18, rel=1e-15
        ), f"pdaw weight[{i}] does not match: delta={weights[i] - reference[i]}"
...
def test_pda_nai(tmp_path):
    ...
    ics.sample_initial_conditions(
        nsamples_ic=10, neg_handling="error", preselect=True, seed=123456789, output_fname=tmp_path / "pda.dat"
    )
    ...
    for i in range(len(pda)):
        ...
        assert pda[1, i] == pytest.approx(
            reference[1, i], abs=1e-18, rel=1e-13
        ), f"pda excitation time for index {i} does not match: delta = {pda[1, i] - reference[1, i]}"
